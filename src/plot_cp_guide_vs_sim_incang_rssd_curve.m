function out = plot_cp_guide_vs_sim_incang_rssd_curve(cfg)
% PLOT_CP_GUIDE_VS_SIM_INCANG_RSSD_CURVE Draws CP guide line vs simulation RSSD scatter.
%
% INPUT
...
if ~isfield(cfg, 'guide') || ~isfield(cfg.guide, 'cp_csv') || ~isfile(cfg.guide.cp_csv)
    error('plot_cp_guide_vs_sim_incang_rssd_curve: guide CSV not found: %s', string(cfg.guide.cp_csv));
end

% Build CP guide line: inc_ang (x) -> mean RSSD (y)
tbl = readtable(cfg.guide.cp_csv, 'VariableNamingRule', 'preserve');
...
hold on;
plot(inc_line, rssd_line, 'k-', 'LineWidth', 2.0, 'DisplayName', 'CP guide (line)');

colors = lines(numel(cfg.scenarios));
for s = 1:numel(cfg.scenarios)
    scenario = cfg.scenarios{s};
    data = load_case_data('CP', scenario, cfg);
    ...
    scatter(inc_sim, rssd_sim, 34, ...
        ...
        'MarkerFaceAlpha', 0.9, ...
        'DisplayName', sprintf('CP sim case%s (scatter)', scenario));
end

grid on;
xlabel('Incidence angle (deg)');
ylabel('RSSD (dB)');
title('CP guide vs simulation inc\_ang-RSSD curve');
legend('Location', 'best');

png_path = fullfile(cfg.results_dir, 'cp_guide_vs_sim_incang_rssd_curve.png');
fig_path = fullfile(cfg.results_dir, 'cp_guide_vs_sim_incang_rssd_curve.fig');
...
error('plot_cp_guide_vs_sim_incang_rssd_curve:columnNotFound', ...
