...
- **Frequency**: 6.5 GHz UWB (sweep 6.24–6.74 GHz, BW = 500 MHz, Δf = 1 MHz, N_freq = 501)
- **Simulation**: HFSS SBR+ ray-tracing (indoor room)
- **System 1 (CP)**: 1 × RHCP anchor + tilted tag with 2 × RHCP antennas
- **System 2 (LP)**: 1 × LP anchor + tilted tag with 2 × LP antennas
...
- **Tag grid**: X ∈ {750, 1500, 2250, 3000, 3750, 4500, 5250} mm (7 cols),
  Y ∈ {−1750, −1250, −750, −250, 250, 750, 1250, 1750} mm (8 rows) → 56 positions total
...
│   └── LP_caseC.csv
├── src/
│   ├── main_track2_1.m          % Top-level runner — runs all stages for all 6 cases
│   ├── setup_config.m           % Returns cfg struct with all parameters
│   ├── load_case_data.m         % Reads one CSV → structured data matrix
│   ├── compute_cir.m            % Complex S21 vector → CIR via windowed IFFT
│   ├── stage1_ranging.m         % CIR → ToA → range error metrics
│   ├── stage2_doa.m             % RSS1/RSS2 → RSSD → DoA error metrics
│   ├── stage3_rejection.m       % CIR → first-path energy ratio
│   ├── stage4_positioning.m     % (range, DoA) → 2D position error metrics
│   ├── compute_metrics.m        % Shared: RMSE, CDF arrays, CEP67, CEP95
│   ├── plot_comparison.m        % CP vs LP overlaid CDF / bar chart per scenario
│   └── generate_summary_table.m % All metrics → summary_table.csv
├── results/                     % All .fig, .png, summary_table.csv written here
├── PLAN.md
├── IMPLEMENT.md
└── DESIGN.md
```
...
| 4            | `mag(S(rx1_p1,tx_p1)) []`       | —     | |S21| magnitude, anchor→tag antenna 1 |
| 5            | `ang_deg(S(rx1_p1,tx_p1)) [deg]`| deg   | ∠S21 phase, anchor→tag antenna 1    |
| 6            | `mag(S(rx2_p1,tx_p1)) []`       | —     | |S21| magnitude, anchor→tag antenna 2 |
| 7            | `ang_deg(S(rx2_p1,tx_p1)) [deg]`| deg   | ∠S21 phase, anchor→tag antenna 2    |

- `tx_p1` = anchor transmit port
- `rx1_p1` = antenna 1 of tilted tag (tilted at +θ_tilt from z-axis)
- `rx2_p1` = antenna 2 of tilted tag (tilted at −θ_tilt from z-axis)

### 4.3 Data Layout in File

Rows are sorted **first by tag position, then by frequency**:
- 501 consecutive rows share the same (x, y)
- 56 × 501 = 28,056 data rows + 1 header = 28,057 lines per file

`load_case_data.m` must reshape the flat rows into a (56 × 501) matrix per antenna.
...
%   data.n_tags     : int           56
%   data.pos_mm     : [56×2]        tag positions [x, y] in mm
%   data.freq_Hz    : [501×1]       frequency axis in Hz
%   data.S21_rx1    : [56×501]      complex S21 for antenna 1
%   data.S21_rx2    : [56×501]      complex S21 for antenna 2
%   data.range_gt_m : [56×1]        ground-truth range = norm(pos_mm - anchor_mm)/1000 [m]
%   data.doa_gt_deg : [56×1]        ground-truth DoA   = atan2d(dy, dx) [deg]
%   data.is_los     : [56×1 logical] true for LoS tags (from cfg.los_idx)
%
% NOTES
...
%
% INPUT
%   S21_vec  : [N_freq×1] complex  — complex S21 at 501 frequency points
%   cfg      : struct               — from setup_config()
%
% OUTPUT
%   cir_mag  : [N_fft×1]  double   — |CIR| magnitude (linear)
%   t_axis_s : [N_fft×1]  double   — time axis [s],  t_n = n/(N_fft * delta_f)
%   d_axis_m : [N_fft×1]  double   — range axis [m], d_n = c * t_n
%
% ALGORITHM
...
%   cfg    : struct — from setup_config()
%
% OUTPUT  (all vectors are [56×1] unless noted)
%   result.range_est_m   : [56×1]  estimated range per tag [m]
%   result.range_gt_m    : [56×1]  ground-truth range [m]  (= data.range_gt_m)
%   result.error_m       : [56×1]  signed ranging error = range_est - range_gt [m]
%   result.abs_error_m   : [56×1]  |error_m|
...
%   result.cdf_x         : [K×1]  CDF x-axis (error values, sorted)
%   result.cdf_y         : [K×1]  CDF y-axis (cumulative probability 0→1)
%   result.pol_type      : char   passed through from data
%   result.scenario      : char   passed through from data
%
% ALGORITHM (per tag i = 1..56)
%   1. S21 = data.S21_rx1(i,:).'   % use antenna 1 for ranging
...
%   cfg    : struct — from setup_config()
%
% OUTPUT  (all vectors [56×1] unless noted)
%   result.rss1_dB       : [56×1]  broadband RSS antenna 1 [dB]
%   result.rss2_dB       : [56×1]  broadband RSS antenna 2 [dB]
%   result.rssd_dB       : [56×1]  RSSD = RSS1 - RSS2 [dB]
%   result.doa_est_deg   : [56×1]  estimated DoA [deg]
%   result.doa_gt_deg    : [56×1]  ground-truth DoA [deg]  (= data.doa_gt_deg)
%   result.error_deg     : [56×1]  signed DoA error = doa_est - doa_gt [deg]
%   result.abs_error_deg : [56×1]  |error_deg|
...
%     rss1_dB(i) = 20*log10( mean(abs(data.S21_rx1(i,:))) )
%     rss2_dB(i) = 20*log10( mean(abs(data.S21_rx2(i,:))) )
%     rssd_dB(i) = rss1_dB(i) - rss2_dB(i)
%
%   Step B — Build RSSD→DoA lookup table (call once per cfg)
...
%
% OUTPUT
%   result.ratio_dB      : [56×1]  MRR = 10*log10(E_fp / E_total) [dB] per tag
...
%
% OUTPUT
%   result.pos_est_mm    : [56×2]  estimated 2D position [mm]
%   result.pos_gt_mm     : [56×2]  ground-truth position [mm]  (= data.pos_mm)
%   result.error_m       : [56×1]  2D Euclidean error [m]
...
%
% INPUT
%   err_vec : [N×1] double  — non-negative error values
%   is_los  : [N×1] logical — LoS mask
%
% OUTPUT
...
%   → 4 stages × 3 scenarios = 12 figures
```
...
data/<POL>_case<X>.csv
  ...
  load_case_data(pol, scenario, cfg)
        |
        +---- data.S21_rx1  [56×501 complex]
        +---- data.S21_rx2  [56×501 complex]
        +---- data.range_gt_m  [56×1]
        +---- data.doa_gt_deg  [56×1]
        +---- data.is_los      [56×1]
        |
        ...
        |
        +=========================> Stage 2: stage2_doa()
        |   per tag: mean(|S21_rx1|), mean(|S21_rx2|) --> rssd_dB
...
```
H_k = mag_k × exp(j × ang_k × π/180)     for k = 1..501
```
...
```
w_k    = 0.5 × (1 − cos(2π(k−1)/(N_freq−1)))     k = 1..N_freq   [Hann window]

H_win  = H .× w                                    [element-wise, N_freq×1]

H_pad  = [ H_win ]                                 [N_fft×1, N_fft = 16384]
         [ zeros ]

h(t)   = IFFT(H_pad, N_fft)                        [complex, N_fft×1]

CIR(t) = |h(t)|                                    [real, N_fft×1]

t_n    = n / (N_fft × Δf)                          n = 0,1,...,N_fft−1
d_n    = c × t_n                                   [range axis, meters]
```

- **Range resolution** (without zero-padding): c/BW = 3×10⁸/5×10⁸ = **0.60 m**
- **Range resolution** (with N_fft=16384):      c/(N_fft×Δf) = 3×10⁸/16.384×10⁹ ≈ **18.3 mm per bin**
...
range_gt_m(i) = sqrt((x_i − x_a)² + (y_i − y_a)²) / 1000     [m]

doa_gt_deg(i) = atan2d(y_i − y_a,  x_i − x_a)                 [deg, −180..180]
```
...
```
RSS1_dB(i) = 20 × log10( mean_f( |S21_rx1(i, :)| ) )
RSS2_dB(i) = 20 × log10( mean_f( |S21_rx2(i, :)| ) )
...
```
θ_tilt = cfg.theta_tilt_deg    [deg]  (e.g. 45°)

For LUT angle φ ∈ [−90°, 90°] at step cfg.rssd_lut_step:

  φ_rad  = φ × π/180
  θ_rad  = θ_tilt × π/180

  G1(φ)  = sin²(φ_rad − θ_rad)      [gain pattern antenna 1 at +θ_tilt]
  G2(φ)  = sin²(φ_rad + θ_rad)      [gain pattern antenna 2 at −θ_tilt]

  RSSD_lut(φ) = 10 × log10( max(G1(φ), ε) / max(G2(φ), ε) )   [dB]

Inverse mapping (per-tag):
  φ_est(i) = interp1(RSSD_lut, φ_lut, RSSD(i), 'linear', 'extrap')   [deg]
```
...
```
φ_est  = doa_est_deg(i) × π/180      [rad]
d_est  = range_est_m(i)              [m]

x_est  = x_a  +  d_est × cos(φ_est) × 1000    [mm]
y_est  = y_a  +  d_est × sin(φ_est) × 1000    [mm]

error_m(i) = sqrt( (x_est − x_gt(i))²  +  (y_est − y_gt(i))² ) / 1000   [m]
```
